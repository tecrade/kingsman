...
const PRICE_RANGES = content["webedit-617"].value;
...
export default function ShopPage() {
  ...
  const FilterPanel = () => <div className="space-y-6">
      ...
      <div>
        <button id="filter-category-toggle" onClick={() => toggleFilterSection('category')} className="flex items-center justify-between w-full text-left mb-3">
          <h3 className="font-serif text-sm font-semibold text-[#f9f6f0] tracking-wide" data-editable="true" data-webedit-id="webedit-560">{content["webedit-560"].value}</h3>
      ...
      <div>
        <button id="filter-tags-toggle" onClick={() => toggleFilterSection('tags')} className="flex items-center justify-between w-full text-left mb-3">
          <h3 className="font-serif text-sm font-semibold text-[#f9f6f0] tracking-wide" data-editable="true" data-webedit-id="webedit-569">{content["webedit-569"].value}</h3>
      ...
      <div>
        <button id="filter-price-toggle" onClick={() => toggleFilterSection('price')} className="flex items-center justify-between w-full text-left mb-3">
          <h3 className="font-serif text-sm font-semibold text-[#f9f6f0] tracking-wide" data-editable="true" data-webedit-id="webedit-576">{content["webedit-576"].value}</h3>
          {expandedFilters.price ? <ChevronUp size={14} className="text-[#f9f6f0]/40" /> : <ChevronDown size={14} className="text-[#f9f6f0]/40" />}
        </button>
        {expandedFilters.price && <div className="space-y-2" data-editable="true" data-webedit-id="webedit-617">
  ...
  return <main id="shop-page" className="min-h-screen bg-[#0a0a0c] pt-28 pb-20">
      <div className="max-w-screen-xl mx-auto px-6">
        {/* Page header */}
        <div className="mb-10">
          <p className="font-mono text-[10px] text-[#c8901a] tracking-[0.35em] uppercase mb-3" data-editable="true" data-webedit-id="webedit-585">{content["webedit-585"].value}</p>
          ...
          {searchParams.get('q') && <p className="text-sm text-[#f9f6f0]/40 font-light" data-editable="true" data-webedit-id="webedit-587">{content["webedit-587"].value}<span className="text-[#c8901a]" data-editable="true" data-webedit-id="webedit-588">{content["webedit-588"].value}{searchParams.get('q')}{content["webedit-588"].value}</span>
            </p>}
        </div>
          ...
          <div className="flex-1 min-w-0">
            {loading ? <div className={`grid gap-5 ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 xl:grid-cols-3' : 'grid-cols-1'}`}>
              ...
              length: 6
                ...
                <p className="font-serif text-2xl text-[#f9f6f0]/40 mb-3" data-editable="true" data-webedit-id="webedit-607">{content["webedit-607"].value}</p>
                <p className="text-sm text-[#f9f6f0]/25 font-light" data-editable="true" data-webedit-id="webedit-608">{content["webedit-608"].value}<button onClick={clearAllFilters} className="text-[#c8901a] hover:underline" data-editable="true" data-webedit-id="webedit-609">{content["webedit-609"].value}</button>
                </p>
      ...
      {filterOpen && <>
          ...
          <aside id="mobile-filter-drawer" className="fixed left-0 top-0 h-full w-72 z-50 bg-[#0f0f12] border-r border-[#c8901a]/15 px-6 py-6 overflow-y-auto animate-slide-in-left lg:hidden">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-serif text-lg font-semibold text-[#f9f6f0]" data-editable="true" data-webedit-id="webedit-614">{content["webedit-614"].value}</h2>
