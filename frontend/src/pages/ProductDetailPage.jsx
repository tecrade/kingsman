...
const SizingGuide = () => <div className="py-6 space-y-5">
    <p className="text-sm text-[#f9f6f0]/60 font-light leading-relaxed" data-editable="true" data-webedit-id="webedit-416">{content["webedit-416"].value}</p>
    ...
    <p className="text-xs text-[#f9f6f0]/30 font-light italic" data-editable="true" data-webedit-id="webedit-425">{content["webedit-425"].value}</p>
  </div>;
const BespokeCrafting = () => <div className="py-6 space-y-5">
    <p className="text-sm text-[#f9f6f0]/60 font-light leading-relaxed" data-editable="true" data-webedit-id="webedit-427">{content["webedit-427"].value}</p>
  ...
  product
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {Object.entries(product.deliveryInfo).map(([key, value]) => <div key={key} className="flex items-start gap-4 p-4 bg-[#1a1a1e] rounded-xl border border-white/6">
          ...
          <div>
            <p className="text-xs font-semibold text-[#f9f6f0] tracking-wide capitalize" data-editable="true" data-webedit-id="webedit-439">{key}{content["webedit-439"].value}</p>
    ...
    <div className="p-4 border border-[#c8901a]/15 rounded-xl bg-[#c8901a]/4">
      <p className="text-xs text-[#f9f6f0]/50 font-light leading-relaxed" data-editable="true" data-webedit-id="webedit-442">{content["webedit-442"].value}</p>
    </div>
  </div>;
...
// ── Tab bar ────────────────────────────────────────────────
const TABS = content["webedit-556"].value;
  ...
  return <main id="product-detail-page" className="min-h-screen bg-[#0a0a0c] pt-20 pb-24">
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-5">
        <nav className="flex items-center gap-2 text-xs text-[#f9f6f0]/30" aria-label="Breadcrumb">
          <Link to="/" className="hover:text-[#c8901a] transition-colors" data-editable="true" data-webedit-id="webedit-453">{content["webedit-453"].value}</Link>
          <ChevronRight size={12} />
          <Link to="/shop" className="hover:text-[#c8901a] transition-colors" data-editable="true" data-webedit-id="webedit-455">{content["webedit-455"].value}</Link>
          <ChevronRight size={12} />
      ...
      <div className="max-w-screen-xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-20">
          ...
          <div className="space-y-4">
            ...
            <div className="relative aspect-[4/5] rounded-2xl overflow-hidden bg-[#1a1a1e] zoom-img">
              <img src={content["webedit-464"].src} alt={content["webedit-464"].alt} className="w-full h-full object-cover object-center" data-editable="true" data-webedit-id="webedit-464" />
              {/* Badges on image */}
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                {product.isNew && <span className="px-3 py-1.5 bg-[#c8901a] text-[#0a0a0c] text-[10px] font-bold tracking-[0.15em] uppercase rounded-full" data-editable="true" data-webedit-id="webedit-466">{content["webedit-466"].value}</span>}
                {product.isBespokeOnly && <span className="px-3 py-1.5 bg-[#f9f6f0] text-[#0a0a0c] text-[10px] font-bold tracking-[0.15em] uppercase rounded-full" data-editable="true" data-webedit-id="webedit-467">{content["webedit-467"].value}</span>}
              </div>
              {/* Prev / Next on image */}
            ...
            <div className="flex gap-3 overflow-x-auto pb-1">
              {product.images.gallery.map((img, i) => <button key={i} id={`gallery-thumb-${i}`} onClick={() => setActiveImage(i)} className={`flex-shrink-0 w-20 h-24 rounded-xl overflow-hidden border-2 transition-all duration-200 ${i === activeImage ? 'border-[#c8901a]' : 'border-transparent opacity-50 hover:opacity-80'}`} aria-label={`View image ${i + 1}`}>
                  <img src={content["webedit-472"].src} alt={content["webedit-472"].alt} className="w-full h-full object-cover object-center" data-editable="true" data-webedit-id="webedit-472" />
                </button>)}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-[#f9f6f0]/50" data-editable="true" data-webedit-id="webedit-481">
                {product.rating}{content["webedit-481"].value}{product.reviewCount}{content["webedit-481"].value}</span>
            </div>
            ...
            <div className="bg-[#141416] border border-white/6 rounded-xl p-5">
              ...
              <div className="space-y-1.5 text-xs text-[#f9f6f0]/40 font-light">
                <div className="flex justify-between">
                  <span data-editable="true" data-webedit-id="webedit-488">{content["webedit-488"].value}</span>
                  <span>{formatCurrency(priceBreakdown.base)}</span>
                </div>
                <div className="flex justify-between text-[#c8901a]/60">
                  <span data-editable="true" data-webedit-id="webedit-491">{content["webedit-491"].value}{(product.luxurySurchargeRate * 100).toFixed(0)}{content["webedit-491"].value}</span>
                  <span>{formatCurrency(priceBreakdown.surcharge)}</span>
                </div>
                <div className="flex justify-between">
                  <span data-editable="true" data-webedit-id="webedit-494">{content["webedit-494"].value}</span>
                ...
                <div className="flex justify-between font-semibold text-[#f9f6f0]/70 pt-1.5 border-t border-white/6">
                  <span data-editable="true" data-webedit-id="webedit-497">{content["webedit-497"].value}</span>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-semibold text-[#f9f6f0] tracking-wide" data-editable="true" data-webedit-id="webedit-504">{content["webedit-504"].value}{' '}
            ...
            <div className="bg-[#141416] rounded-xl border border-white/6 overflow-hidden">
              <div className="px-5 py-4 border-b border-white/6">
                <p className="font-serif text-sm font-semibold text-[#f9f6f0] tracking-wide" data-editable="true" data-webedit-id="webedit-528">{content["webedit-528"].value}</p>
              </div>
        ...
        <div className="mt-20">
          <div className="border-b border-white/8 mb-0">
            <div className="flex gap-0 overflow-x-auto" data-editable="true" data-webedit-id="webedit-556">
          ...
          <div className="mt-12 border-t border-white/6 pt-6">
            <p className="font-mono text-[10px] text-[#f9f6f0]/20 tracking-[0.3em] uppercase mb-4" data-editable="true" data-webedit-id="webedit-543">{content["webedit-543"].value}</p>
        ...
        {related.length > 0 && <div className="mt-20">
            <div className="flex items-center justify-between mb-8">
              <h2 className="font-serif text-2xl font-bold text-[#f9f6f0]" data-editable="true" data-webedit-id="webedit-553">{content["webedit-553"].value}</h2>
