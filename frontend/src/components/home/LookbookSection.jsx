  ...
  return <section id="lookbook" className="py-24 bg-[#0d0d0f]" aria-label="Lookbook">
      <div className="max-w-screen-xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16 items-end">
          <div>
            <p className="font-mono text-[10px] text-[#c8901a] tracking-[0.35em] uppercase mb-4" data-editable="true" data-webedit-id="webedit-71">{content["webedit-71"].value}</p>
            <h2 className="font-serif text-4xl md:text-5xl font-bold text-[#f9f6f0] leading-tight" data-editable="true" data-webedit-id="webedit-72">{content["webedit-72"].value}<br />
              <span className="italic text-[#c8901a]" data-editable="true" data-webedit-id="webedit-74">{content["webedit-74"].value}</span>
            </h2>
          </div>
          <div className="lg:max-w-sm lg:ml-auto">
            {/* Editorial drop-cap paragraph */}
            <p className="drop-cap text-sm text-[#f9f6f0]/50 leading-loose font-light" data-editable="true" data-webedit-id="webedit-76">{content["webedit-76"].value}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="lg:col-span-7 relative group zoom-img rounded-xl overflow-hidden cursor-pointer" onMouseEnter={() => setHoveredId(lookbookEntries[0].id)} onMouseLeave={() => setHoveredId(null)}>
            <div className="aspect-[4/5] lg:aspect-auto lg:h-[600px]">
              <img src={content["webedit-80"].src} alt={content["webedit-80"].alt} className="w-full h-full object-cover object-center" loading="lazy" data-editable="true" data-webedit-id="webedit-80" />
            </div>
            {/* Overlay */}
          ...
          <div className="lg:col-span-5 flex flex-col gap-5">
            {lookbookEntries.slice(1).map(entry => <div key={entry.id} className="relative group zoom-img rounded-xl overflow-hidden cursor-pointer flex-1" onMouseEnter={() => setHoveredId(entry.id)} onMouseLeave={() => setHoveredId(null)}>
                <div className="aspect-video lg:aspect-auto lg:h-[285px]">
                  <img src={content["webedit-92"].src} alt={content["webedit-92"].alt} className="w-full h-full object-cover object-center" loading="lazy" data-editable="true" data-webedit-id="webedit-92" />
                </div>
                <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0c]/80 via-transparent to-transparent" />
                <div className="absolute bottom-0 left-0 right-0 p-5">
                  ...
                  <Link to="/shop" id={`lookbook-cta-${entry.id}`} className={`inline-flex items-center gap-2 mt-3 text-[10px] font-semibold tracking-[0.15em] uppercase text-[#c8901a] transition-all duration-300 ${hoveredId === entry.id ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`} data-editable="true" data-webedit-id="webedit-97">{content["webedit-97"].value}<ArrowUpRight size={12} />
                  </Link>
                </div>
              </div>)}
          </div>
