...
const NAV_LINKS = content["webedit-365"].value;
  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'glass shadow-lg shadow-black/20' : 'bg-transparent'}`}>
      <div className="max-w-screen-xl mx-auto px-6 flex items-center justify-between h-16 lg:h-20">
        ...
        <Link to="/" id="nav-logo" className="flex items-center gap-2.5 group">
          ...
          <span className="font-serif text-xl lg:text-2xl font-bold tracking-[0.18em] text-[#f9f6f0] uppercase" data-editable="true" data-webedit-id="webedit-348">{content["webedit-348"].value}</span>
        </Link>

        {/* Desktop Nav */}
        <nav className="hidden lg:flex items-center gap-1" data-editable="true" data-webedit-id="webedit-365">
