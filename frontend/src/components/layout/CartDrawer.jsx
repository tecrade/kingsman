      ...
      <aside id="cart-drawer" role="dialog" aria-modal="true" aria-label="Shopping cart" className={`fixed right-0 top-0 h-full w-full max-w-md z-50 flex flex-col bg-[#141416] border-l border-[#c8901a]/15 shadow-2xl shadow-black/60 transition-transform duration-350 ease-in-out ${cartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/8">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-serif text-lg font-semibold text-[#f9f6f0] tracking-wide" data-editable="true" data-webedit-id="webedit-162">{content["webedit-162"].value}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto py-4 px-6">
          {cartItems.length === 0 ? <div className="flex flex-col items-center justify-center h-full gap-5 text-center">
              ...
              <div>
                <p className="font-serif text-lg text-[#f9f6f0]/60" data-editable="true" data-webedit-id="webedit-169">{content["webedit-169"].value}</p>
                <p className="text-sm text-[#f9f6f0]/30 mt-1 font-light" data-editable="true" data-webedit-id="webedit-170">{content["webedit-170"].value}</p>
              </div>
              <button onClick={() => setCartOpen(false)} className="px-6 py-2.5 border border-[#c8901a]/40 text-[#c8901a] text-sm font-medium tracking-[0.1em] uppercase rounded hover:bg-[#c8901a]/10 transition-colors" data-editable="true" data-webedit-id="webedit-171">{content["webedit-171"].value}</button>
              ...
              {cartItems.map(item => <li key={item.key} className="flex gap-4 pb-5 border-b border-white/6 group">
                  ...
                  <div className="w-20 h-24 rounded-lg overflow-hidden flex-shrink-0 bg-[#1a1a1e]">
                    <img src={item.product.images.front} alt={item.product.name} className="w-full h-full object-cover object-center" />
                  </div>
                  {/* Details */}
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#f9f6f0]/40 mt-1" data-editable="true" data-webedit-id="webedit-180">{content["webedit-180"].value}{item.size}{content["webedit-180"].value}{item.color}
                    </p>
        ...
        {cartItems.length > 0 && <div className="border-t border-white/8 px-6 py-5 space-y-3 bg-[#0f0f12]">
            <div className="space-y-2 text-sm">
              <div className="flex justify-between text-[#f9f6f0]/60">
                <span data-editable="true" data-webedit-id="webedit-195">{content["webedit-195"].value}</span>
                <span>{formatCurrency(cartTotals.subtotal)}</span>
              </div>
              <div className="flex justify-between text-[#c8901a]/80">
                <span data-editable="true" data-webedit-id="webedit-198">{content["webedit-198"].value}</span>
                <span>{formatCurrency(cartTotals.luxurySurcharge)}</span>
              </div>
              <div className="flex justify-between text-[#f9f6f0]/60">
                <span data-editable="true" data-webedit-id="webedit-201">{content["webedit-201"].value}</span>
              ...
              <div className="flex justify-between font-semibold text-[#f9f6f0] text-base pt-2 border-t border-white/8">
                <span className="font-serif" data-editable="true" data-webedit-id="webedit-204">{content["webedit-204"].value}</span>
          ...
          setCheckoutOpen(true);
            ...
            <p className="text-center text-xs text-[#f9f6f0]/25 tracking-wide" data-editable="true" data-webedit-id="webedit-207">{content["webedit-207"].value}</p>
          </div>}
      </aside>
    </>;
}