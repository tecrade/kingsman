...
const MOBILE_LINKS = content["webedit-333"].value;
      ...
      <aside id="mobile-nav-drawer" role="dialog" aria-modal="true" aria-label="Navigation menu" className={`fixed left-0 top-0 h-full w-72 z-50 flex flex-col bg-[#0f0f12] border-r border-[#c8901a]/15 shadow-2xl shadow-black/60 transition-transform duration-350 ease-in-out lg:hidden ${mobileNavOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/8">
          <Link to="/" onClick={() => setMobileNavOpen(false)} className="flex items-center gap-2">
            ...
            <span className="font-serif text-lg font-bold tracking-[0.18em] text-[#f9f6f0] uppercase" data-editable="true" data-webedit-id="webedit-323">{content["webedit-323"].value}</span>
          </Link>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-4">
          <ul className="space-y-1" data-editable="true" data-webedit-id="webedit-333">
        ...
        <div className="px-6 py-5 border-t border-white/8">
          <p className="text-[10px] text-[#f9f6f0]/20 tracking-[0.15em] uppercase mb-1" data-editable="true" data-webedit-id="webedit-331">{content["webedit-331"].value}</p>
          <p className="text-xs text-[#f9f6f0]/35 font-light" data-editable="true" data-webedit-id="webedit-332">{content["webedit-332"].value}</p>
        </div>
      </aside>
    </>;
}