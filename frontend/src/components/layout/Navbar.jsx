...
const NAV_LINKS = [
  ...
  },
  {
    ...
    dropdown: [
      ...
      { label: 'Book a Consultation', href: '/shop?category=Bespoke' },
      { label: 'Bespoke Archive', href: '/shop?tag=Archive' },
    ],
  },
  { label: 'Lookbook', href: '/#lookbook', dropdown: null },
  { label: 'Heritage', href: '/#heritage', dropdown: null },
];

export default function Navbar() {
  const { cartCount, setCartOpen, setMobileNavOpen, setCheckoutOpen, setCheckoutType } = useApp();
    ...
    >
      <div className="max-w-screen-xl mx-auto px-6 flex items-center justify-between h-16 lg:h-20">
        ...
        <button
          ...
          className="lg:hidden text-[#f9f6f0] p-1"
        ...
        <Link to="/" id="nav-logo" className="flex items-center gap-2.5 group">
          <div className="relative">
            <Crown
              size={28}
              className="text-[#c8901a] group-hover:scale-110 transition-transform duration-300"
              fill="#c8901a"
              strokeWidth={1}
            />
          </div>
          <span
            className="font-serif text-xl lg:text-2xl font-bold tracking-[0.18em] text-[#f9f6f0] uppercase"
          >
            Kingsman
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={searchRef}>
            <button
              ...
              className="p-2 text-[#f9f6f0]/80 hover:text-[#c8901a] transition-colors"
              aria-label="Search"
            >
            ...
            {searchOpen && (
              <form
                ...
                className="absolute right-0 top-full mt-2 w-72 glass rounded-lg border border-[#c8901a]/20 shadow-xl shadow-black/40 overflow-hidden"
              >
                <div className="flex items-center px-4 py-3 gap-3">
                  ...
                  <input
                    ...
                    autoFocus
                    className="flex-1 bg-transparent text-[#f9f6f0] placeholder-[#f9f6f0]/30 text-sm outline-none tracking-wide"
                  />
                </div>
              </form>
            )}
          ...
          <button
            ...
            className="relative p-2 text-[#f9f6f0]/80 hover:text-[#c8901a] transition-colors"
            aria-label="Open cart"
          >
            <ShoppingBag size={20} />
            {cartCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] bg-[#c8901a] text-[#0a0a0c] text-[10px] font-bold rounded-full flex items-center justify-center px-1 animate-pulse-gold">
                {cartCount}
              </span>
            )}
          </button>
